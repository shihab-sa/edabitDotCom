...
function availableSpots(arr, num) {
  ...
  for (let i = 0; i < arr.length - 1; i++) {
    const leftNeighbor = arr[i];
    const rightNeighbor = arr[i + 1];

    if ((isOdd(num) && (isEven(leftNeighbor) || isEven(rightNeighbor))) ||
        (isEven(num) && (isOdd(leftNeighbor) || isOdd(rightNeighbor)))) {
      likedSpots++;
    }
  }
...
// Test cases
console.log(availableSpots([0, 4, 6, 8], 9));   // Output: 0
console.log(availableSpots([0, 4, 6, 8], 12));  // Output: 3
console.log(availableSpots([4, 4, 4, 4, 5], 7)); // Output: 1
console.log(availableSpots([4, 4], 8));         // Output: 1
